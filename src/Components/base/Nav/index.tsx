...
const NavBar = (props: IProps) => {
	...
	const listenScrollEvent = () => {
		// Nếu collapsed => cho phép hiển thị
		if (collapsed) {
			setIsDisplay(true);
		} else {
			setIsDisplay(false);
	...
	return (
		<>
			<Disclosure
				as="nav"
				className={`${
					hiddenNav ? 'opacity-0 pointer-events-none' : 'opacity-100'
				} bg-white fixed top-0 left-0 w-full z-[99999] py-3 lg:py-6 transition-all duration-300 shadow-md`}
			>
			...
			<div className={` ${collapsed && styles.slideDown} lg:hidden z-10`}>
				<div
